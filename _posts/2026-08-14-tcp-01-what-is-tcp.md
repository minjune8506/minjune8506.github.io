...
- **연결지향**: 양쪽이 먼저 "연결" 상태를 만들어야 데이터를 주고받을 수 있고, 다 끝나면 명시적으로 끊습니다. (연결을 맺는 법은 3편에서 다룹니다.)
- **신뢰성**: 보낸 데이터가 순서대로, 빠짐없이 도착하도록 보장합니다. 유실되면 재전송하고, 순서가 뒤바뀌면 재조립합니다. (구체적인 방법은 4~5편에서 다룹니다.)
...
TCP는 이 전부를 대신 해주는 하나의 "패키지"입니다. 사실 이 시리즈 남은 글들은 이 목록을 하나씩 뜯어보는 것과 다르지 않습니다 — 4편은 흐름제어, 5편은 재전송·재조립, 6편은 혼잡제어입니다.
