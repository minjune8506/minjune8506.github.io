...
## 곁다리: Window가 너무 잘게 쪼개지면 — Silly Window Syndrome

윈도우 크기가 작아지는 게 항상 안전하지는 않습니다. 만약 수신자가 버퍼를 아주 조금씩(예: 1바이트씩) 비우면서 그때마다 "1바이트 더 받을 수 있어"라고 알려주면 어떻게 될까요? 송신자는 그 1바이트를 위해 헤더 20바이트짜리 세그먼트를 통째로 만들어 보내야 합니다. 데이터 1바이트에 오버헤드가 20배 이상 붙는 셈입니다. 이 비효율을 **Silly Window Syndrome**이라고 부릅니다.

해결책은 양쪽에 다 있습니다. 수신자는 윈도우가 일정 크기 이상으로 늘어날 때까지 "더 받을 수 있다"는 신호를 미룹니다. 송신자도 마찬가지로, 보낼 데이터가 아주 적으면 (Nagle 알고리즘) 어느 정도 모일 때까지 기다렸다가 한 번에 보냅니다. 둘 다 "티끌 모아 태산"으로 세그먼트를 만들어서, 자잘한 세그먼트가 남발되는 걸 막는 겁니다.
</div>
