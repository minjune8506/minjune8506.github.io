...
<svg viewBox="0 0 640 160" role="img" aria-label="시퀀스 넘버 축 위에서 데이터는 전송 완료(ACK 받음), 전송함-ACK 대기중, 전송 가능(window 안), 전송 불가(window 밖) 네 영역으로 나뉜다. 가운데 두 영역(전송함-ACK 대기중, 전송 가능)을 합친 크기가 수신 윈도우다." xmlns="http://www.w3.org/2000/svg">
...
ACK가 도착할 때마다 "ACK 받음" 영역이 오른쪽으로 늘어나고, 그만큼 윈도우 전체가 오른쪽으로 밀립니다. 그래서 이름이 **슬라이딩** 윈도우입니다.

윈도우 크기(rwnd) 자체는 고정이 아니라, 수신자가 매 ACK마다 "지금 내 버퍼 여유가 이만큼이야"라고 새로 알려주는 값입니다.
...
`window=0`을 받으면 송신자는 그냥 멈춥니다. 문제는 여기서 끝나면 안 된다는 겁니다 — 수신자가 나중에 버퍼를 비워서 "이제 받을 수 있어"라고 알리고 싶어도, 그 ACK 자체가 유실되면 송신자는 영원히 멈춘 채로 남습니다.

그래서 송신자는 주기적으로 **window probe**(1바이트짜리 작은 탐침 세그먼트)를 보내서 "아직도 0이야?"라고 물어봅니다. 수신자가 응답하면서 window 값을 실어 보내고, 그 값이 0보다 커지면 송신자는 전송을 재개합니다.

<div class="tangent" markdown="1">
## Window가 너무 잘게 쪼개지면 — Silly Window Syndrome

윈도우 크기가 작아지는 게 항상 안전하지는 않습니다. 만약 수신자가 버퍼를 아주 조금씩(예: 1바이트씩) 비우면서 그때마다 "1바이트 더 받을 수 있어"라고 알려주면 어떻게 될까요?

송신자는 그 1바이트를 위해 헤더 20바이트짜리 세그먼트를 통째로 만들어 보내야 합니다. 데이터 1바이트에 오버헤드가 20배 이상 붙는 셈입니다. 이 비효율을 **Silly Window Syndrome**이라고 부릅니다.

해결책은 양쪽에 다 있습니다. 수신자는 윈도우가 일정 크기 이상으로 늘어날 때까지 "더 받을 수 있다"는 신호를 미룹니다.

송신자도 마찬가지로, 보낼 데이터가 아주 적으면 (Nagle 알고리즘) 어느 정도 모일 때까지 기다렸다가 한 번에 보냅니다. 둘 다 "티끌 모아 태산"으로 세그먼트를 만들어서, 자잘한 세그먼트가 남발되는 걸 막는 겁니다.
</div>
